...
const Store = createStore({
  ...
  mutations: {
    ...
    prepare (state, info) {
      Object.keys(info).forEach(prop => {
        if (Object.keys(state).includes(prop) && ALLOW_FOR_PREPARE.includes(prop))
  ...
  actions: {
    ...
    async prepare_app (cxt) {
      ...
      if (data.success)
        cxt.commit('prepare', data.result)
      else
        cxt.commit('show_error', 'Не удалось подготовить приложение')
    },
  },
